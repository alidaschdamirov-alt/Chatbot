...
def analyze_calendar_image_openai(png_path: Path, api_key: str, model="gpt-4o-mini") -> str:
    ...
    try:
        ...
        system_msg = (
            ...
            "1) ключевые релизы, 2) Актуальные показатели со скрина - Последний показатель, Прогноз, Предыдущий 3) импликации для решения ФРС по ставке, "
            "3) влияние на BTC/крипту, 4) на индекс доллара (DXY), 5) на акции (S&P/Nasdaq), 6) риски."
        )
        user_msg = (
            "Формат ответа:\n"
            "• Ключевые релизы и актуальные показатели\n• Ставка ФРС\n• BTC/крипто\n• DXY\n• Акции\n• Риски"
        )
